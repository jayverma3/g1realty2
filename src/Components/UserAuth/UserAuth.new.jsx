...
export default function UserAuth() {
  ...
  const handleRegister = async (e) => {
    e && e.preventDefault();
    clear();
    if (!email || !password || !name) return setError("Name, email and password are required.");
    setLoading(true);
    const { ok, data } = await postJson("/auth/register", { name, email, password, phone });
    setLoading(false);
  ...
  const openPopup = (url) => {
    ...
    return window.open(url, "oauth", `width=600,height=700,left=${left},top=${top}`);
  };
    ...
    <div className="ua-wrapper">
      <div className="ua-card" role="region" aria-label="User authentication">
        <div className="ua-left">
          <div className="brand">
            <h1>Account</h1>
            <p className="muted">Access your profile, manage listings and save favorites.</p>
          </div>

          <div className="tabs" role="tablist" aria-label="Authentication tabs">
            <button className={mode === "login" ? "active" : ""} onClick={() => { clear(); setMode("login"); }}>Login</button>
            <button className={mode === "register" ? "active" : ""} onClick={() => { clear(); setMode("register"); }}>Register</button>
            <button className={mode === "phone" ? "active" : ""} onClick={() => { clear(); setMode("phone"); }}>Phone</button>
          </div>
          ...
          <div className="form">
            ...
            {mode === "phone" && (
              <form onSubmit={otpSent ? handleVerifyOtp : handleSendOtp}>
                ...
                <div className="actions">
                  <button className="btn primary" type="submit" disabled={loading}>{loading ? "Please wait..." : (otpSent ? "Verify OTP" : "Send OTP")}</button>
                  <button type="button" className="btn ghost" onClick={handleGoogle}>Continue with Google</button>
                </div>
              </form>
            )}

            <div className="note">By continuing you agree to our terms. Backend must implement secure storage in MySQL and token issuance.</div>
          </div>
        </div>

        <div className="ua-right" aria-hidden>
          <div className="illustration">
            <svg viewBox="0 0 480 320" xmlns="http://www.w3.org/2000/svg" aria-hidden>
              <defs>
                <linearGradient id="g" x1="0" x2="1"><stop offset="0" stopColor="#e6f2ff"/><stop offset="1" stopColor="#f2fbff"/></linearGradient>
              </defs>
            ...
            <div className="illu-text">
              ...
              <p className="muted">Sign in with Google, email/password, or phone OTP. Connect this UI to your Hostinger MySQL-backed API to store users safely.</p>
            </div>
          </div>
        </div>
      </div>
